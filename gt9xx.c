...
#include <linux/irq.h>
//#include "gt9xx.h"
//#include "gt9xx_gt5688_720x1280.h"
#include "gt9xx_gt5688_800x1280.h"
#include <linux/pm.h>
...
//    GTP_DEBUG("ID:%d, X:%d, Y:%d, W:%d", id, x, y, w);
	dprintk(DEBUG_INIT,"[%s: %d] ID:%d, X:%d, Y:%d, W:%d==================exit\n",__func__,__LINE__, id, x, y, w);
}
...
static void gtp_pen_init(struct goodix_ts_data *ts)
{
    ...
    input_set_abs_params(ts->pen_dev, ABS_MT_POSITION_X, 0, screen_max_x, 0, 0);
    input_set_abs_params(ts->pen_dev, ABS_MT_POSITION_Y, 0, screen_max_y, 0, 0);
...
static void goodix_ts_work_func(struct work_struct *work)
{
    ...
    u8  touch_num = 0;
    u8  finger = 0;
	printk("================gt9xxnew_ts goodix_ts_work_func===============\n");
...
//#endif
    ...
    #if GTP_HAVE_TOUCH_KEY
	printk("================GTP_HAVE_TOUCH_KEY======11111111==========pre_touch = %d\n", pre_touch);
        if (!pre_touch)
        {
